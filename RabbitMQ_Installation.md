...
Ref: https://www.rabbitmq.com/docs/install-debian <br/>
youtube: https://www.youtube.com/watch?v=N-AqOeaP8Ag

Script:

```
touch rabbitmq_setup.sh
code rabbitmq_setup.sh
```
...
change the mode of the file and execute:

```
chmod +x rabbitmq_setup.sh
./rabbitmq_setup.sh
```
...
```
it should be active: Active: active (running),
if not enbale and start:
...
launch the application on browser (RabbitMQ Management server) 
default: 
```
